...
<%@ page import="com.listing.bean.LisBean" %>
<%
    LisBean house = (LisBean) request.getAttribute("house");
    List<EquipmentBean> equipmentList = (List<EquipmentBean>) request.getAttribute("equipmentList");
    List<Integer> selectedEquipIds = (List<Integer>) request.getAttribute("selectedEquipIds");
%>
        ...
        <form action="UpdateListing" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3 row">
                <div class="col">
                    ...
                    <input type="text" name="room" class="form-control"
                           value="<%= house != null ? house.getRoom() : "" %>">
                </div>
                <div class="col">
                    <label class="form-label">床位</label>
                    <input type="text" name="bed" class="form-control"
                           value="<%= house != null ? house.getBed() : "" %>">
                </div>
            </div>

            <div class="mb-3">
                <label class="form-label">描述</label>
                <textarea name="describe" class="form-control" rows="3"><%= house != null ? house.getDescribe() : "" %></textarea>
            </div>
            ...
            <div class="mb-3">
                ...
                <div class="d-flex flex-wrap gap-2">
                    <% if (house != null) {
                        for(int i=1; i<=10; i++) {
                            String photo = (String) LisBean.class.getMethod("getPhoto"+i).invoke(house);
                            if(photo != null && !photo.trim().isEmpty()) {
                    %>
                    <img src="<%= photo %>" alt="photo<%=i%>" style="width:100px; height:70px; object-fit:cover; border-radius:8px;">
                    <%      }
                        }
                    } %>
                </div>
            ...
            <div class="mb-3">
                ...
                <div class="row">
                    <% if (equipmentList != null) {
                        for (EquipmentBean eq : equipmentList) {
                            boolean checked = selectedEquipIds != null && selectedEquipIds.contains(eq.getEquip_id());
                    %>
                    <div class="col-6 col-md-4 mb-2">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="equipments" value="<%= eq.getEquip_id() %>" <%= checked ? "checked" : "" %> >
                            <label class="form-check-label"><%= eq.getEquip_name() %></label>
                        </div>
                    </div>
                    <%  }
                    } %>
